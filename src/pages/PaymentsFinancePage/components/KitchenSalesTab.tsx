...
export function KitchenSalesTab() {
  ...
  const [rows, setRows] = useState<CoffeeSale[]>([])
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const itemName = String(appliedFilters.item_name ?? '').trim()
        if (itemName) {
        ...
        const response = await fetch(`${API_BASE_URL}/coffee/sales?${params.toString()}`, {
          headers: {
  ...
  const handleSort = (columnKey: keyof CoffeeSale) => {
  ...
  const columns: Array<DataTableColumn<CoffeeSale>> = useMemo(
    () => [
      {
        key: 'coffee_sale_id',
        label: 'ID',
  ...
  return (
    <>
      ...
      <DataTable
        ...
        onSort={(columnKey) => handleSort(columnKey as keyof CoffeeSale)}
      />
